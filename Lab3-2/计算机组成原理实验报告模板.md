...
3. 加入由 `MemtoReg` 控制的二选一多路选择器，选择写入寄存器的数据来源（恒定常数 32’b1 或 ALU 结果）。向 `t1`、`t2` 写入常数 1，通过仿真验证写入正确性。
4. 在 `$t1`、`$t2` 已写入初值的基础上，模拟执行 `add $t0, $t1, $t2`：从 `$t1`、`$t2` 读出数据送至 ALU 做加法，结果通过 `MemtoReg=0` 写回 `$t0`，仿真验证加法结果正确性。
