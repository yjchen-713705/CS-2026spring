...
2. **Test 2**：`5 - 3`  
   ...
   - 无符号解释：结果正确，5 - 3 = 2
   - 有符号解释：同样为 2，结果正确
   - cout=1 表示产生进位，但不等价于借位（补码减法中被减数大于减数）

3. **Test 3**：`9 + 7`     <!-- TODO:写成二进制 -->
   输出：1001 + 0111 = 0000，cout=1
   - 无符号解释：实际应为 9 + 7 = 16，4位系统截断为 0000，产生进位 cout=1
   - 有符号解释（补码）：1001 = -7，运算实际为：-7 + 7 = 0
   - 结论：无符号下发生溢出（结果被截断），有符号下结果“看似正确”，说明同一输入在不同编码体系下意义完全不同

4. **Test 4**：`14 - 15`   <!-- TODO:写成二进制 -->
   输出：sum=1111，cout=0
