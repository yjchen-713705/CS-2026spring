...
2. `seg7.v`：七段数码管显示模块，将32位结果转换为七段数码管显示的格式，注意低电平有效
```verilog
`timescale 1ns / 1ps
...
module seg7(
    ...
    always@(*)
    case(din)
    // 通过din实现七段数码管显示的转换（点亮需要的段），通过dout输出七段数码管的显示格式
...
3. `display.v`：数码管显示模块，将ALU结果显示到七段数码管上，注意选择逻辑低电平有效
>
- 为什么要分频？
    1. **更好的视觉效果**：FPGA开发板的原始时钟信号为100MHz，直接用于数码管扫描会导致扫描速度过快，人眼无法感知显示内容，可以利用人眼的视觉暂留特性，将时钟分频至1kHz。这个频率远高于视觉暂留阈值，可以确保显示稳定。
    2. **控制扫描节奏**：通过分频控制sel信号的递增速度，实现8个数码管的依次扫描显示，避免信号冲突。
    3. **降低系统功耗**：高频时钟会增加系统功耗，分频后可以有效降低功耗。
...
module display(
    ...
    reg [20:0]count;     // 用于时钟分频
    reg [2:0] sel;       // 数码管选择信号
    reg [3:0] digit;     // 要显示的4位数据
    reg [31:0] s_reg;    // 存储结果的寄存器
    
    // 时钟分频
    // 方法：使用计数器count，当count达到100000时，将count重置为0，sel递增1，实现1kHz的时钟
...
<img src="pics/000-1.png" alt="加法运算1" style="width: 60%;" />

2. num1 = 0xFF，预期结果0x00000002 (3+(-1)=2)
<img src="pics/000-2.png" alt="加法运算2" style="width: 60%;" />

#### 2. 减法运算 op = 001

1. num1 = 0x01，预期结果0x00000002 (3-1=2)
<img src="pics/001-1.png" alt="减法运算1" style="width: 60%;" />

2. num1 = 0xFF，预期结果0x00000004 (3-(-1)=4)
<img src="pics/001-2.png" alt="减法运算2" style="width: 60%;" />

#### 3. 按位与运算 op = 010

1. num1 = 0x03，预期结果0x00000003 (3&3=3)
<img src="pics/010-1.png" alt="按位与运算1" style="width: 60%;" />

2. num1 = 0x01，预期结果0x00000001 (3&1=1)
<img src="pics/010-2.png" alt="按位与运算2" style="width: 60%;" />

#### 4. 按位或运算 op = 011

1. num1 = 0x04，预期结果0x00000007 (3|4=7)
<img src="pics/011-1.png" alt="按位或运算1" style="width: 60%;" />

2. num1 = 0x01，预期结果0x00000003 (3|1=3)
<img src="pics/011-2.png" alt="按位或运算2" style="width: 60%;" />

#### 5. 取反运算 op = 100

1. num1 = 任意值，预期结果0xFFFFFFFC (~3=0xFFFFFFFC)
<img src="pics/100-1.png" alt="取反运算1" style="width: 60%;" />

#### 6. 比较大小运算 op = 101

1. num1 = 0x04，预期结果0x00000001 (3<4，输出1)
<img src="pics/101-1.png" alt="比较大小运算1" style="width: 60%;" />

2. num1 = 0x02，预期结果0x00000000 (3<2，输出0)
<img src="pics/101-2.png" alt="比较大小运算2" style="width: 60%;" />
...
2. 问题描述：拍摄实验板时频闪导致显示不清晰        
   解决方法：通过手机截图（这样做比较方便，实际应该也可以通过修改数码管的扫描频率避开相机帧率整数倍关系来解决）
