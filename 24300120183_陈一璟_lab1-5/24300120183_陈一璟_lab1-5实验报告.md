...
2. 数码管显示（16进制）：根据选择信号 sel，将 s_reg 中的 4 位数据 digit 显示到七段数码管 seg 中。由于16进制数码管，所以每四位二进制数据对应一个七段数码管。

```verilog
   // output wire [7:0]ans // 七段数码管显示的4位数据
   // reg [2:0] sel;       // 数码管选择信号
