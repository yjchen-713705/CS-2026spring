...
- `regfile.v`：寄存器堆，支持同步写、组合读。
- `alu.v`：算术逻辑单元，支持 add/sub/and/or/slt 等运算。
- `mux2.v`：二选一选择器（参数化宽度）。
- `adder.v`：32 位加法器（用于 PC+4 和分支地址计算）。
...
10. IP核：使用 Block Memory Generator 生成指令 ROM（深度 256，宽度 32，单端口 ROM，不勾选 Primitive Output Registers）和数据 RAM（深度 256，宽度 32，单端口 RAM）。
